...
const createSummary = (data: Data) => {
  ...
  return [createHeading1('Summary'), ...createParagraphsByLine(data.summary)];
};
      ...
      .flatMap((group) => {
        ...
        return filteredTechnologies.length > 0
          ? [
              ...
              new Paragraph({
                children: [...filteredTechnologies, new TextRun({ text: '.' })],
              }),
            ]
          : undefined;
      })
...
const createEducations = (data: Data) => {
  ...
  return [
    ...
    ...data.educations.flatMap((education) => [
      createHeading3WithRightText(
        education.school,
        formatDates(education.startDate, education.endDate).replace(
          /\s/g,
          NBSP,
        ),
      ),
...
const createHeading3WithRightText = (text: string, rightText: string) =>
  new Paragraph({
